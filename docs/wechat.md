<!--
Module doc for WeChat.
Sections: current status, change log, important files, next steps.
Dependencies: src/App.tsx, src/store.ts, src/index.css, src/lib/charaParser.ts, src/lib/utils.ts.
Maintenance note: update this file whenever WeChat UI/state/import behavior changes.
-->

# 微信模块

## 当前状态

- 微信入口位于桌面 app，内部保留 4 个底部页签：微信、通讯录、发现、我。
- 微信聊天列表和通讯录都直接读取全局 `characters`，导入角色卡后会同步出现在微信聊天列表和微信通讯录。
- 聊天主界面支持文字消息、图片消息、带描述的表情包消息、语音条消息、浏览器 TTS 播放、语音未听红点、长按/右键/按钮查看语音转写；长按消息可进入多选模式，用小圆点选择多条后统一删除。
- 聊天加号面板支持 NAI AI 生图；角色 AI 可以低频输出 `[image prompt="..."]` 主动发图片，图片会同步进入系统相册。
- 聊天 API 未配置时会明确提示缺少接口地址或模型，不再用固定本地回复冒充已连接 API。
- “RP 长短”分为短 RP、长 RP 和自动：短 RP 会连续显示几条短气泡；长 RP 会把完整回复和自然分段保留在同一个气泡；括号动作或心理描写只在确有需要时低频出现。接口空正文或达到输出长度上限时会显示明确原因，不再静默等待。
- 通讯录保留新的朋友、群聊、标签；群聊可选择联系人创建，标签可给联系人填写。
- 发现页包含朋友圈、照片墙和内置表情包库；朋友圈是好友动态流，默认先看动态，点“发朋友圈”才展开发布器，支持图文发布、可见范围、角色自动回复、点赞、评论、单条刷回复、进入 char 的朋友圈主页，以及从 char 设定里提取好友身份生成评论；表情包可上传并编辑文字描述。
- 我页面支持修改头像、昵称、状态、微信号；默认微信号为 `9142`，状态输入移到列表行里，避免覆盖个人资料区。
- 微信样式跟随全局主题：微信内部背景、面板、文字、弱文字、强调色、气泡、红点和入口图标都从外层主题变量派生，避免单独冒出不协调的红黄蓝绿。

## 本次改动

- 2026-05-10：微信界面主题收敛到外层主题色。`--wechat-*` 变量改为读取 `--phone-bg`、`--phone-text`、`--panel-bg` 和 `--accent`；聊天列表、通讯录、我页箭头、入口图标、语音红点、生活卡片、撤回/收藏工具和输入栏按钮不再各自使用跳色。
- 2026-05-11：微信加号面板新增 NAI AI 生图；AI 生活动作解析新增 `[image prompt="..."]`，模型发图会生成图片气泡并同步相册；输入框聚焦和移动端 visual viewport 变化时会滚动到最新消息，避免键盘呼出后看不到聊天记录。
- 2026-05-10：`WeChatChats`、`WeChatContacts`、`WeChatMe` 去掉硬编码文字/箭头色，改用 `wechat-row-title`、`wechat-row-preview`、`wechat-row-time`、`wechat-row-chevron` 等语义 class；`ChatScreen` 的微信聊天背景和发送按钮改由微信主题变量控制。
- 2026-08-13：主题页新增微信/QQ共用的自定义聊天背景，图片写入 IndexedDB 后由聊天房间解析稳定引用；没有自定义图片时继续跟随整机主题。微信“我 → 记账与订单”改为跳转桌面独立记账 App，仍读取同一份旧订单数据。
- 2026-05-10：微信聊天语音条新增未听红点。模型发来的语音消息默认显示红点，点击播放后通过 `markVoiceMessagePlayed` 写入 `voicePlayedAt` 并隐藏红点；用户自己发出的语音不显示未听红点。
- 2026-05-10：新增 `src/apps/wechat/chat/voiceUnread.ts` 和 `VoiceMessageBubble.tsx`，把语音未读判断、播放态、转文字按钮和红点 UI 从 `ChatScreen.tsx` 拆出，避免聊天房间继续变厚。
- 2026-06-23：新增 `src/apps/wechat/chat/components/ChatBubble.tsx`，把文字、图片、表情、语音、通话提示和转账/红包/购物生活卡片气泡从 `ChatScreen.tsx` 拆出；`ChatScreen.tsx` 只保留聊天流程、输入栏、发送和 AI 回复编排。
- 2026-06-23：朋友圈深化互动层。用户发朋友圈时会自动生成角色回复；每条朋友圈可点赞/取消点赞、手动评论、单条“刷回复”，并在评论区展示点赞人和评论内容。
- 2026-06-23：朋友圈改为好友动态流。角色发的动态可点击头像/名字进入 TA 的朋友圈主页；主页显示封面、签名和从角色设定提取的好友身份标签；如果设定里有摄影社朋友、室友、同学、同事等关系，会作为设定好友出现在评论区。
- 2026-06-23：朋友圈发布入口改为收起式。进入朋友圈默认先看好友动态；点“发朋友圈”展开发布器，发表成功后自动收起，避免一进来就像必须先发动态。
- 2026-05-10：Zustand persist 版本升级到 44，`ChatMessage` 增加兼容可选字段 `voicePlayedAt`，迁移时保留旧消息并补齐语音播放状态字段。
- 2026-05-08：微信四个底部页签从 `src/App.tsx` 拆到 `src/apps/wechat/`：`chats/WeChatChats.tsx`、`contacts/WeChatContacts.tsx`、`discover/WeChatDiscover.tsx`、`me/WeChatMe.tsx`。`App.tsx` 现在只保留微信外壳 `WeChatApp`、聊天房间 `ChatScreen` 和气泡 `Bubble`。
- 2026-05-08：新增 `src/apps/wechat/shared/WeChatShared.tsx`，集中微信页签共享的顶栏、头像、群头像、消息预览、聊天预设和预设解析，避免四个页签互相复制。
- 2026-05-08：聊天页继续按 `ChatScreen` / `Bubble` 局部修整。AI 接口失败不再写入“接口出错”聊天气泡，改为输入栏上方的重试条；用户连发后增加“输入框留空点发送，对方再回复”的待回复提示。
- 2026-05-08：新增 `src/apps/wechat/wechatChat.ts` 存放微信回复拆泡/清洗逻辑，去掉编号、旁白行和角色名前缀，限制 AI 回复更像短微信气泡；保留原有预设、上下文深度、温度、maxTokens、replyStyle 设置。
- 2026-05-08：微信聊天里的语音/视频通话入口仍只写入 `call-note` 消息，不触碰电话模块；`Bubble` 对 `call-note` 使用居中通话提示，不再像普通文字气泡。
- 2026-05-08：图片文件名显示做截断，长文本/引用/工具按钮补充换行与溢出保护，加号面板增加高度限制，避免移动端横向滚动或顶出屏幕。
- 2026-05-08：微信聊天新增生活卡片：转账、红包、购物；角色 AI 也可以低频主动发生活动作或表情包。新增 `src/apps/wechat/ai/` 存放微信 AI 提示词、生活动作解析和新手教程。
- 2026-05-08：新增 `src/apps/wechat/stickers/` 表情包 starter manifest，只使用 OpenMoji、Noto Emoji、Twemoji 这类 emoji/sticker 风格素材，不引入斗图包。
- 2026-05-08：群聊不再显示“群名 正在输入中”；群聊行支持长按或右键修改群名。通讯录和发现页入口色块调整为更协调的低饱和配色。
- 2026-05-08：Zustand persist 版本升级到 38，`ChatMessage` 增加转账/红包/购物可选字段：`amount`、`note`、`itemName`、`status`。
- 将兼容 ID 生成集中在 `src/lib/utils.ts`，不再读取 `randomUUID` 字段，避免旧环境导入酒馆卡时报错。
- 整理 `src/lib/charaParser.ts` 的 `createId` import，保持角色卡导入链路清晰。
- 微信通讯录导入成功提示改为明确说明已同步到微信聊天列表。
- 微信聊天空列表的“导入角色卡”入口改为切到微信内部通讯录页签，不再跳到全局旧通讯录。
- Zustand persist 版本升级到 13，旧持久化数据会重新迁移：空微信号或旧 `Muon0417` 会覆盖为 `9142`，微信页不再保留旧状态文案。
- 微信页、消息列表、贴纸栏继续隐藏浏览器滚动条，并补充输入栏/底部页签的横向溢出防护。
- 哥特主题进入微信/聊天时隐藏全局玻璃状态栏覆盖，并强制微信标题使用真实微信式字体；奶油主题微信列表去掉大圆角粗边卡片，改为更接近微信的列表分隔。
- 通讯录删除“仅聊天的朋友、公众号、服务号”，群聊改为可创建群聊，标签改为可给联系人写标签。
- 发现页新增朋友圈；表情包库支持上传图片并给每个表情填写描述。
- 聊天输入栏默认不再露出四个大方块，改为点击表情按钮展开面板。
- 微信聊天列表遇到表情消息时显示“表情：描述”，不再显示 `data:image...`。
- 通讯录联系人没有标签时只显示名字；用户输入标签后才显示标签行。
- 发现页改为“朋友圈 / 照片墙 / 表情包”三条入口，点入后再编辑内容。
- “我”页删除照片墙和表情包入口，避免和发现页重复。
- 通讯录入口图标降低橙/绿/蓝/红饱和度。
- 表情包上传必须先填写描述，再点击上传。
- 表情包卡片支持收藏和删除；聊天消息支持收藏和删除。
- 主界面奶油主题颜色调整为更低饱和的绿、蓝、黄、粉。
- 聊天消息补充复制、引用回复、撤回；引用会带入下一条发送内容。
- “我”页新增收藏页，汇总已收藏消息和已收藏表情。
- “订单与卡包”变成 char 购买记录，可记录购买角色、物品、金额和备注。
- 设置页文本大模型支持保存接口地址/API Key、拉取模型列表、选择模型；聊天优先走 OpenAI 兼容接口。
- 聊天输入栏新增图片发送；接口失败时保留重试条；语音条播放中会显示波形动效。
- 微信聊天改为连续发送模式：有内容时先发送用户气泡，输入框空白再按发送才触发 char 回复。
- char 回复会按微信设置里的“RP 长短”显示为短 RP 多气泡、长 RP 单气泡或自动选择；微信“我 > 设置”还包含内置聊天预设、上下文深度、温度和最大长度。
- 假语音输入框占位字缩小并固定一行。
- 消息操作改为点击/长按气泡后呼出，不再每条常驻；用户消息只保留引用和撤回，char 消息可引用、复制、收藏、撤回或删除。
- 消息补充发送时间；char 回复时显示“正在输入中”。接口完整返回后会一次性保存本轮全部气泡，避免逐条延迟落库造成尾部截断。
- 输入栏右侧收紧为加号面板，表情、图片、语音通话、视频通话统一从加号展开。
- 群聊会同步出现在微信聊天列表，通讯录里可点开群聊，也可解散群聊。
- 发现详情页左上返回只回到发现入口，不再直接退出微信。
- 微信聊天预设在桌面“预设”App 中导入和管理，支持 `.json/.txt` 文件或粘贴酒馆式预设文本，并映射 name/prompt/temperature/depth/max_tokens 等字段。酒馆 JSON 会完整保留 `prompts` 与 `prompt_order` 的条目顺序、开关和 system/user/assistant 身份；预设 App 支持搜索、筛选、新增、展开编辑、显式保存、上下移动、单独开关和删除，只有开启且有正文的条目会进入请求。微信“我 > 设置”只显示当前预设、条目摘要、跳转入口和聊天生成参数。
- 微信底部页签红点改为只显示未读数，不再因为存在历史会话一直显示。
- 用户消息操作按钮区分为绿色“引用”和红色“撤回”，不再两个箭头长得像。
- 输入栏左侧语音按钮删除；语音条、表情、图片、语音通话、视频通话都收进发送旁边的加号面板。
- 表情包描述不再显示在聊天气泡里，只作为“表情包注释”进入 AI 上下文。
- AI 请求会带入角色人设、性格、开场白、系统提示和世界书；群聊请求会带入所有群成员人设，并要求 AI 同时扮演多人。
- 通讯录联系人点击先进入朋友资料页，资料页提供朋友圈、发消息、音视频通话入口；标签编辑时不会误进聊天。
- 群聊解散按钮改成红色，并增加二次确认；群聊头像改为成员头像拼格。
- 预设导入适配 SillyTavern/酒馆格式，会读取 `prompts`、`temperature`、`openai_max_tokens`、`openai_max_context`、模型名等字段。
- char 消息不再提供“撤回”，改为删除；用户消息可引用、收藏、复制、撤回。
- 引用按钮改为双引号图标；撤回按钮保持红色，和引用明确区分。
- 群聊回复改为每个成员按自己人设各发消息，不再把多人合成一个人格。
- 微信与 QQ 共用同一个聊天预设；内置预设只保留“小手机 · 短 RP”和“小手机 · 长 RP”，上下文消息数可调到 1000。
- 通讯录标签改成标签筛选逻辑：先给联系人设置标签，再点标签 chip 筛选联系人。
- 聊天页顶部右侧语音按钮已删除，语音条只保留在加号面板里。
- 2026-06-15：文字、语音、图片、表情、转账、红包和购物发送后默认触发角色回复；待回复草稿会从历史上下文中排除，避免同一条消息重复进入 AI。微信 AI 允许低频输出 `[image prompt="..."]` 主动发图，一次回复最多一张。
- 2026-06-15：微信聊天发送前会检查聊天 API 连接，缺少接口地址或模型时在重试条/后台记录里显示具体原因，不再写入固定本地回复让玩家误以为 API 已连接。
- 2026-06-15：聊天 API 返回 400/401/429/500 等 HTTP 错误时，重试条会直接显示红色 `HTTP xxx` 错误码；网络超时这类没有 HTTP 响应的情况则显示真实网络错误。
- 2026-06-20：聊天 API 未配置时只在重试条和后台记录显示“缺少接口地址/模型”，不写入任何默认本地回复；HTTP 错误改为 `HTTP 429/500 ...` 前置显示完整返回详情。用户手动 AI 生图不再自动触发角色第二轮回复，避免“发图后又重复回我”；角色主动生图和今日生活主动生图统一进入 NAI gate，命中角色/频道/用户冷却、每日额度、失败重试或重复 prompt 时只写日志和状态，不继续发图。
- 2026-07-05：聊天气泡新增多选删除模式。长按任意聊天消息进入选择态，消息旁显示小圆点；继续点消息或圆点切换多选；输入栏上方显示“已选择 N 条”和统一删除按钮。批量删除支持用户消息和角色消息，删除待回复用户消息时同步清理待回复草稿。
- 2026-08-06：主题 App 新增独立“气泡美化”，提供跟随主题、微信青绿、QQ 晴空、雾面玻璃、手账贴纸和像素电波；微信与 QQ 共用选择，文字与语音条同步变化。该选择只改外观，不影响微信设置里的 `chatReplyStyle` 拆泡方式。
- 2026-08-07：修正回复截断链路。后台返回的完整正文会先解析，再一次性写入所有气泡，不再边等待边逐条保存；JSON 数组外壳不会显示成 `[`，`reasoning_content` 不会被当作正文。短 RP 保留多条短气泡，长 RP 保留完整单气泡，括号描写仅低频允许。默认最大回复长度从 520 升到 1200；文本接口地址兼容 Gemini `/v1beta/openai` 和完整 `/chat/completions`。
- 2026-08-11：短 / 长 RP 统一由“正在使用”下拉框选择，删除“自动判断”及自然微信、活人感微信、AI助手、黏人连发、克制冷淡等旧内置项；两条 RP 规则在条目列表中永远只有当前模式开启。括号风格同组只能开启一个。微信与 QQ 的预设名称、条目、提示词和回复模式完全共用。

## 现实聊天功能表

| 类别 | 功能 | 当前状态 | 下一步 |
| --- | --- | --- | --- |
| 基础消息 | 文字发送、连续发送、自动触发回复、时间、滚动到底、发送失败重试、防重复历史 | 已有 | 多选管理 |
| 语音 | 语音条、TTS、转文字、播放中动画、未听红点 | 已有 | 更细的触摸反馈 |
| 表情包 | 内置表情、自定义上传、AI 描述、发送 | 已有 | 表情分组、搜索 |
| 生活动作 | 转账、红包、购物卡片、角色主动低频发送、低频主动发图 | 初版 | 收款状态、更多生活事件 |
| 消息操作 | 点击呼出操作、长按进入多选、用户撤回、char 删除、批量删除、收藏、复制、双引号引用 | 已有 | 转发 |
| 联系人 | 导入角色卡、朋友资料页、标签筛选 | 已有 | 备注名、星标朋友、黑名单 |
| 群聊 | 创建群聊、聊天列表同步、拼格头像、点开群聊、解散群聊、每成员逐个发言 | 初版 | 群聊成员选择性发言 |
| 朋友圈 | 好友动态流、发朋友圈按钮展开发布器、char 朋友圈主页、图文发表、心情/装饰、可见范围、角色/设定好友评论、点赞、手动评论、单条刷回复 | 已有 | 评论回复楼中楼、图片大图预览、自定义 char 封面 |
| 媒体 | 照片墙、聊天发图片 | 初版 | 相册选择、图片理解 |
| 资料 | 头像、昵称、状态、微信号 | 已有 | 二维码、个性签名 |
| 收藏 | 收藏消息/表情、独立收藏页 | 已有 | 收藏搜索、分类 |
| 预设 | 微信/QQ 共用短 RP 或长 RP；桌面“预设”App 导入并管理酒馆条目 | 已有 | 角色独立预设 |
| 搜索 | 通讯录搜索入口 | 占位 | 搜消息、联系人、朋友圈 |
| 安全/隐私 | 标签和关系分层 | 初版 | 仅聊天、权限、隐藏会话 |
- 角色卡导入和世界书显示增加常见乱码修复。
- Zustand persist 版本升级到 18，迁移旧字符串表情为带描述的表情对象，补齐朋友圈、群聊、标签、订单与模型配置字段，并清理旧 4 月 30 示例日记。

## 重要文件

- `src/apps/wechat/WeChatApp.tsx`：微信四页签外壳。
- `src/apps/wechat/chat/ChatList.tsx`：微信/QQ 共用聊天列表入口。
- `src/apps/wechat/chat/ChatScreen.tsx`：微信/QQ 共用聊天房间流程，负责输入栏、发送、AI 回复编排、失败重试、加号面板和消息多选删除状态。
- `src/apps/wechat/chat/components/ChatBubble.tsx`：微信/QQ 共用消息气泡 UI，负责文字、图片、表情、语音、通话提示、生活卡片渲染和多选圆点入口。
- `src/apps/wechat/chat/VoiceMessageBubble.tsx`：微信语音条 UI，包含播放态、未听红点和转文字开关。
- `src/apps/wechat/chat/voiceUnread.ts`：语音消息未读判断和播放后标记逻辑。
- `src/apps/wechat/chats/WeChatChats.tsx`：微信聊天列表。
- `src/apps/wechat/contacts/WeChatContacts.tsx`：微信通讯录、导入角色卡、群聊、标签。
- `src/apps/wechat/discover/WeChatDiscover.tsx`：发现页、朋友圈、照片墙、表情包。
- `src/apps/wechat/me/WeChatMe.tsx`：我页、收藏、订单与卡包、微信聊天预设设置。
- `src/apps/wechat/shared/WeChatShared.tsx`：微信页签共享 UI 和格式化/预设工具。
- `src/apps/wechat/ai/`：微信 AI 活人感提示词、生活动作解析和新手教程。
- `src/apps/wechat/stickers/`：emoji/sticker 风格表情包 starter manifest 和素材来源说明。
- `src/store.ts`：微信个人资料字段、照片墙、朋友圈、表情包描述、群聊、联系人标签、聊天记录、旧数据迁移。
- `src/index.css`：微信浅色/暗色主题、聊天气泡、底部页签、滚动条隐藏。
- `src/lib/charaParser.ts`：酒馆 PNG/JSON 角色卡解析。
- `src/lib/utils.ts`：兼容 ID 生成。

## 下一步

- 给语音条增加更细的触摸反馈。
- 给聊天输入栏增加更多面板和相册选择。
- 群聊下一步接入真正的群成员消息上下文和成员管理。
- 接下来可做转发、群聊房间、图片理解、收藏分类和角色独立预设。
- 后续如继续细拆，可把 `ChatBubble` 内的消息操作条、生活卡片和媒体气泡再拆到更小组件。

## 2026-07-18 v1.12 聊天交互回归

- 小剧场进入聊天后使用 `kind: "theater"` 的剧情卡；兼容旧版 `call-note + 【小剧场】`，两者都支持展开、复制、收藏、单条删除和长按多选删除。
- 聊天默认只渲染最近 200 条，顶部每次加载更早 100 条；历史数据不裁剪、不自动删除。
- 键盘高度只由 App 壳的 `visualViewport` 写入 `--app-vvh`，聊天页不再重复计算 lift；微信/QQ 输入栏和加号编辑面板按可视高度滚动，不叠加两次底部偏移。
- `ChatScreen` 和 `ChatBubble` 使用精确 store selector，`ChatBubble` 使用 `React.memo`，避免任意全局状态变化重绘全部消息。
