...
2026-06-15 主动事件补充 `send_image` 建议。它只从当天聊天或可读相册提取真实线索，刷新时只生成建议和 NAI prompt；用户点“确认写入”后才调用 NAI，成功后写入微信图片消息、同步保存到相册，并记录生活事件。刷新本身不后台生图、不自动消耗额度。若 NAI 失败，只写入错误日志和状态，不把建议标记为已确认。

2026-06-22 主动事件新增“随机主动”本地开关。开启后，小手机运行期间会低频读取角色人设作息、主动程度和轻量生活事件草稿，向微信私聊写入自然主动消息，并同步写入 `lifeEvents`。`char主动` 页提供“立即测试随机主动”，通过 `manual_test` 模式立刻写入一条测试消息，方便确认文风和写入链路；它不打开自动随机，也不改变后台常驻策略。地点报备只在人设支持旅行、跑外勤、巡演、采风、流浪或频繁换地点时生成；普通固定生活角色不会凭空跨城跨国。第一版不后台常驻、不弹锁屏通知、不自动生图。

2026-06-15 User 信息支持多玩家档案。当前启用的 User/玩家档案会作为角色记忆上下文的一部分进入微信 AI；多个档案之间彼此独立，切换档案会同步昵称和 User 信息。该档案描述的是“玩家当前身份”，不等同于 char 角色卡，也不替代各 char 的世界书或人设。

2026-06-23 User 信息支持点击玩家头像更换本机头像。头像写入全局 `userAvatar`，用于微信、QQ 等界面展示玩家身份；头像图片本身不作为 AI 人设文本发送，避免把视觉素材误当成角色设定或长期记忆。
